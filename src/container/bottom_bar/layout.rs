...
macro_rules! allowed {
    ($ty:ty, $func:expr $(,)?) => {{
        let f = |any: Box<dyn Any + Send + Sync>| -> StyledString {
            let any = any as Box<dyn Any + Send>;
            $func(*any.downcast::<$ty>().unwrap())
        };
...
#[rustfmt::skip]
static ALLOWED_STRING_TYPES: &[(Type, fn(Box<dyn Any + Send + Sync>) -> StyledString)] = &[
    // Base types:
...
impl Layout {
    ...
    fn stringify(any: Box<dyn Any + Send + Sync>) -> StyledString {
        let input_type = (&*any).type_id();
        //                ^^^^^ This dereference here is so that we can guarantee we get a
        //                `&dyn Any` from the original reference. If we don't, the TypeId will
        //                just be equal to the TypeId of `Box<dyn Any + ...>`, which is certainly
        //                not what we want.
        ...
        panic!("unexpected input type (id {:?})", input_type);
    }
}
