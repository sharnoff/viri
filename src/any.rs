...
//! but that's about it. The benefits from this range from better error messages (`Type`), to
//! simpler type definitions (`BoxedAny`, mostly).
//!
...
/// A replacement for [`std::any::TypeId`] with type names built-in
///
...
///
/// `Type`s are generated by the [`new`](Self::new) method.
...
/// A wrapper around `Box<dyn Any + Send + Sync>` with the concrete type of the value attached
///
/// The type of the underlying value is exposed through through the [`inner_type`] method, and
/// downcasting is provided with [`downcast`]. There is also [`try_downcast`], for a fallible
/// version that will not panic.
///
